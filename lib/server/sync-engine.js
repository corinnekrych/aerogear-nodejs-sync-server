...
var ServerSyncEngine = function () {
    ...
    // TODO: these should be an adapter or something
    var stores = {
        docs: [],
        shadows: [],
        backups: [],
    ...
    this.addSubscriber = function (subscriber, doc) {
        console.log('TODO: ADD SUBSCRIBER STUFF');
        return this.addDocument(doc, subscriber.clientId);
    };

    this.addDocument = function (doc, clientId) {
            var existing, existingDoc, shadowDoc, patchMessage, shadowCopy, magic;

        if (!doc.content) {
            existingDoc = this.getdoc(doc.id);
            ...
            if (!existingDoc.length) {
                ...
                shadowDoc = this.addShadowForClient(doc.id, clientId);

                magic = this._seededShadowFrom(shadowDoc, doc);
                patchMessage = this.serverDiff(shadowDoc, magic);
        ...
        existing = this.saveDocument(doc);

        // Add Shadow for the client
        shadowDoc = this.addShadowForClient(doc.id, clientId);

        if (existing) {
            console.log('Document with ID: ' + doc.id + 'exists already');
            magic = this._seededShadowFrom(shadowDoc, doc);
            patchMessage = this.serverDiff(shadowDoc, magic);
        } else {
            shadowCopy = utils.poorMansCopy(shadowDoc);
            shadowCopy.serverVersion++;
            patchMessage = this.serverDiff(shadowDoc, shadowCopy);
        }
    ...
    this.addShadowForClient = function (docId, clientId) {
        this.addShadow(docId, clientId, 0);
    };

    this.addShadow = function (docId, clientId, clientVersion) {
